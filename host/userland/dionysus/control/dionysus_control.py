...
def usage():
  ...
  print ""
  print "usage: dionysus-control.py [options] <filename>"
  print ""
...
def main(argv):
  os.environ["S1_BASE"] = sys.path[0] + "/dc-pycontrol"
  ...
  dc = Dionysus_Control(idVendor = 0x0403, idProduct = 0x8530)
  ...
  for opt, arg in opts:
    if opt in ("-h", "--help"):
      ...
      dc = Dionysus_Control(idVendor = vid, idProduct = pid) 
  ...
  if (not dc.write_bin_file(args[0])):
    print "Failed!"
    sys.exit(3)
