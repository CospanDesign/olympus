...
class PropertyView(gtk.Frame):
  def __init__(self):
    ...
    # Separator.
    sep = gtk.HSeparator()
    self.vbox.pack_start(sep, expand = False, fill = True, padding = 0)
...
#                  self.parameters)
    # Go through each of the properties and overwrite the previous value.
  ...
  def initialize_table(self):
    ...
    self.table.attach(self.module_name_entry, 1, 2, 0, 1,
                      xoptions = gtk.EXPAND | gtk.FILL,
                      yoptions = gtk.EXPAND | gtk.FILL,
                      xpadding = self.txpad,
                      ypadding = self.typad)
    ...
    self.table.attach(self.file_name_entry, 1, 2, 1, 2,
                xoptions = gtk.EXPAND | gtk.FILL,
                yoptions = gtk.EXPAND | gtk.FILL,
                xpadding = self.txpad,
                ypadding = self.typad)
  ...
  def set_property(self, name, value, editable):
    ...
    self.table.attach(label, 0, 1, self.pos - 1, self.pos,
              xoptions = gtk.EXPAND | gtk.FILL,
              yoptions = gtk.EXPAND | gtk.FILL,
              xpadding = self.txpad,
              ypadding = self.typad)
    ...
    self.table.attach(entry, 1, 2, self.pos - 1, self.pos,
              xoptions = gtk.EXPAND | gtk.FILL,
              yoptions = gtk.EXPAND | gtk.FILL,
              xpadding = self.txpad,
              ypadding = self.typad)
